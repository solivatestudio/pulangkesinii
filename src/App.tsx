...
export default function App(){
  ...
  return <div className="page-stage"><a className="skip-link" href="#kegiatan">Lewati ke daftar kegiatan</a><main className="mobile-shell">
    ...
    <section className="moments-section" aria-labelledby="moments-title"><div className="moments-heading"><div className="simple-heading"><span>Cerita dalam gambar</span><h2 id="moments-title">Momen Kebaikan</h2></div><AppleEmoji code="2728" alt="Berkilau"/></div><p>Potret perjalanan, pertemuan, dan kebaikan yang pernah kami jalani bersama.</p><div className="moments-rail">{galleryPhotos.map((photo,index)=>{const tileClass=index===3?'featured':index===8||index===11?'wide':index===13?'tall':'';return <button key={photo} className={tileClass} onClick={()=>setGalleryImage(photo)} aria-label={`Buka foto dokumentasi ${index+1}`}><img src={photo} alt={`Dokumentasi kegiatan Pulangkesinii ${index+1}`} loading="lazy"/></button>})}</div><small>Geser untuk melihat semua foto · ketuk untuk memperbesar</small></section>
