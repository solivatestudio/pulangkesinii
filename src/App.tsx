...
import { CalendarDays, ChevronDown, ChevronRight, Globe2, HandHeart, Handshake, Heart, HelpCircle, Instagram, Linkedin, Mail, MapPin, MessageCircle, Route, Search, Tag, X } from 'lucide-react';
...
function AppleEmoji({ code, alt='' }: { code:string; alt?:string }) {
  return <img className="ios-emoji" src={`/assets/emoji/${code}.png`} alt={alt} width="22" height="22"/>;
}
...
export default function App(){
  ...
  return <div className="page-stage"><a className="skip-link" href="#kegiatan">Lewati ke daftar kegiatan</a><main className="mobile-shell">
    ...
    <section className="moments-section" aria-labelledby="moments-title"><div className="moments-heading"><div className="simple-heading"><span>Cerita dalam gambar</span><h2 id="moments-title">Momen Kebaikan</h2></div><AppleEmoji code="2728" alt="Berkilau"/></div><p>Potret perjalanan, pertemuan, dan kebaikan yang pernah kami jalani bersama.</p><div className="moments-rail">{galleryPhotos.map((photo,index)=><button key={photo} className={index===3?'featured':''} onClick={()=>setGalleryImage(photo)} aria-label={`Buka foto dokumentasi ${index+1}`}><img src={photo} alt={`Dokumentasi kegiatan Pulangkesinii ${index+1}`} loading="lazy"/></button>)}</div><small>Geser untuk melihat semua foto · ketuk untuk memperbesar</small></section>
    ...
    <section className="values-section"><div className="simple-heading"><span>Yang kami jaga</span><h2>Nilai Pulangkesinii</h2></div><div className="value-grid"><span><AppleEmoji code="1f49b"/>Empathy</span><span><AppleEmoji code="1f91d"/>Collaboration</span><span><AppleEmoji code="1f331"/>Growth</span><span><AppleEmoji code="1f4ab"/>Impact</span><span><Heart/>Integrity</span></div></section>
    <section className="vision-section"><div><AppleEmoji code="1f9ed"/><span>Visi</span><p>Menjadi komunitas sosial yang menginspirasi generasi muda untuk bertumbuh, berbagi, dan menciptakan dampak positif melalui aksi nyata yang berkelanjutan.</p></div><details><summary>Misi Pulangkesinii <ChevronDown/></summary><ol><li>Menjadi ruang aman bagi anak muda untuk berkembang melalui kegiatan sosial.</li><li>Mendorong budaya volunteer yang inklusif dan menyenangkan.</li><li>Menghubungkan individu dan komunitas dalam kolaborasi sosial.</li><li>Menghadirkan program sosial yang kreatif dan edukatif.</li><li>Membangun kepedulian sosial sebagai bagian dari gaya hidup.</li></ol></details></section>
  ...
  {legal&&<div className="modal-backdrop" onMouseDown={()=>setLegal(null)}><section className="legal-sheet" role="dialog" aria-modal="true" onMouseDown={e=>e.stopPropagation()}><button className="sheet-close" onClick={()=>setLegal(null)} aria-label="Tutup"><X/></button><AppleEmoji code="1f3e1"/><h2>{legal==='terms'?'Syarat & Ketentuan':'Kebijakan Privasi'}</h2><p>Dokumen resmi masih menunggu verifikasi tim Pulangkesinii. Halaman ini disiapkan sebagai state sementara dan tidak menetapkan kebijakan baru.</p><a href="mailto:pulangkesinii@gmail.com">Hubungi Pulangkesinii</a></section></div>}
  </div>;
}
