  ...
  return <div className="space-y-5">
    ...
    {tab === 'fields' && <div className="space-y-5">
      <div className="bg-white p-5 rounded-2xl border grid md:grid-cols-2 gap-4 text-xs">
        ...
        <label>Metode Pembayaran<input value={config.paymentMethods.join(', ')} onChange={(e) => setConfig({...config, paymentMethods:e.target.value.split(',').map((v)=>v.trim()).filter(Boolean)})} className="block w-full h-10 px-3 mt-1 border rounded-xl" /><small>Pisahkan dengan koma</small></label>
      </div>
